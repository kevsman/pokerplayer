...
from improved_preflop_ranges import create_improved_preflop_ranges
from improved_postflop_engine import create_improved_postflop_engine
...
class EnhancedPokerBot(PokerBot):
    ...
    def __init__(self, config_path='config.json'):
        ...
        try:
          ...
          # Ensure parser exists before initializing action detector
        ...
        # Ensure action_history exists for test environment
        ...
        # Initialize improved strategy modules
        self.improved_preflop_ranges = create_improved_preflop_ranges()
        self.improved_postflop_engine = create_improved_postflop_engine()
        ...
        try:
            while True:
                ...
                if game_analysis['my_turn']:
                    self.logger.info("My turn detected - making improved strategic decision")
                    decision_result = self._make_improved_strategic_decision(game_analysis)
                    if decision_result:
    ...
    def _make_enhanced_decision(self, game_analysis: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            action, amount, reasoning = self.decision_engine_advanced.make_advanced_decision(context)            # CRITICAL SAFEGUARD: Never fold when check is available
            ...
            # IMPROVED POT ODDS SAFEGUARD: Don't fold when getting good pot odds
            elif action == 'fold' and bet_to_call > 0 and pot_odds > 0:
                # Calculate required equity to break even
                required_equity = pot_odds
                # If our win probability is reasonably close to required equity, call instead
                if win_probability >= (required_equity * 0.8):  # Call if we have at least 80% of required equity
                    action = 'call'
                    amount = bet_to_call
                    reasoning += f" [POT ODDS SAFEGUARD: {win_probability:.1%} vs {required_equity:.1%} required - calling]"
                    self.logger.warning(f"POT ODDS SAFEGUARD: Changing fold to call. Win prob: {win_probability:.2%}, Required: {required_equity:.2%}")
            
            # POSITION SAFEGUARD: Be more aggressive in late position when it's cheap
            elif action == 'fold' and my_player.get('position', '') in ['BTN', 'CO'] and bet_to_call > 0:
                # If it's a small bet relative to pot in good position, consider calling
                if bet_to_call <= (pot_size * 0.3) and win_probability >= 0.25:  # 25% win rate with small bet
                    action = 'call'
                    amount = bet_to_call
                    reasoning += f" [POSITION SAFEGUARD: Good position + small bet - calling]"
                    self.logger.warning(f"POSITION SAFEGUARD: Calling in {my_player.get('position')} with {win_probability:.2%} win rate")
    ...
    def _execute_enhanced_action(self, decision_result: Dict, game_analysis: Dict):
        ...
        try:
            ...
            if action == ACTION_FOLD:
                ...
                if amount > 0:
                    # Bet action - similar to raise but might be initial bet
                    self.ui_controller.action_raise(amount)
                    self.logger.info(f"BET {amount:.2f} executed - {reasoning}")
                else:
                    # Default bet with no amount specified
                    self.ui_controller.action_check_call()
                    self.logger.info(f"BET (check) executed - {reasoning}")
            else:
                # Handle any unrecognized action as check
                self.ui_controller.action_check_call()
                self.logger.warning(f"Unknown action '{action}', executing CHECK - {reasoning}")
    ...
    def _make_improved_strategic_decision(self, game_analysis: Dict) -> Optional[Dict]:
        """
        Make decision using improved preflop ranges and postflop engine that considers betting history.
        """
        ...
        try:
            ...
            stack_str = my_player.get('stack', '0')
            if isinstance(stack_str, str):
                stack_str = stack_str.replace('€', '').replace('$', '').replace(',', '').strip()
            try:
                stack_size = float(stack_str)
            except ValueError:
                stack_size = 0.0
                
            pot_size = table_data.get('pot_size', 0.0)
            game_stage = table_data.get('game_stage', 'preflop').lower()
            position = my_player.get('position', 'BB')
            available_actions = [action['type'] for action in game_analysis.get('actions_available', [])]
              # Get hole cards if available - FIXED: Check both 'cards' and 'hand' fields
            hole_cards = my_player.get('cards', []) or my_player.get('hand', [])
            if isinstance(hole_cards, str):
                # Try to parse string representation
                hole_cards = hole_cards.split() if ' ' in hole_cards else []
            
            # Additional validation and logging for hole cards
            if hole_cards and len(hole_cards) >= 2:
                self.logger.info(f"Successfully detected hole cards: {hole_cards}")
            else:
                self.logger.warning(f"No hole cards detected. my_player['cards']: {my_player.get('cards', 'missing')}, my_player['hand']: {my_player.get('hand', 'missing')}")
            
            big_blind = table_data.get('big_blind', 1.0)
            
            self.logger.info(f"Improved decision: Stage={game_stage}, Position={position}, "
                           f"Cards={hole_cards}, Bet2Call={bet_to_call}, Pot={pot_size}, "
                           f"WinProb={win_probability:.2%}, Stack={stack_size}")
            
            # PREFLOP STRATEGY
            if game_stage == 'preflop' and hole_cards and len(hole_cards) == 2:
                action, multiplier, reasoning = self.improved_preflop_ranges.get_preflop_action_recommendation(
                    hole_cards=hole_cards,
                    position=position,
                    bet_to_call=bet_to_call,
                    big_blind=big_blind,
                    action_history=self.action_history,
                    active_opponents=game_analysis.get('active_opponents_count', 1)
                )
                
                # Calculate actual bet amount
                if action == 'raise':
                    amount = multiplier * big_blind
                    # Ensure minimum raise requirements
                    if bet_to_call > 0:
                        # This is a re-raise, ensure we meet minimum requirements
                        min_raise_total = bet_to_call + bet_to_call  # Simple 2x rule
                        amount = max(amount, min_raise_total)
                    # Cap at stack size
                    amount = min(amount, stack_size)
                elif action == 'call':
                    amount = bet_to_call
                else:
                    amount = 0.0
                ...
                if action == 'fold' and 'check' in available_actions:
                    action = 'check'
                    amount = 0.0
                    reasoning += " [SAFEGUARD: Changed fold to check]"
                    self.logger.warning(f"SAFEGUARD ACTIVATED: Changed fold to check when check was available.")
                
                elif action == 'fold' and bet_to_call == 0.0:
                    action = 'check'
                    amount = 0.0
                    reasoning += " [SAFEGUARD: Changed fold to check when no bet]"
                    self.logger.warning(f"SAFEGUARD ACTIVATED: Changed fold to check when bet_to_call was 0.")
                
                # POT ODDS SAFEGUARD: Don't fold when getting good pot odds
                elif action == 'fold' and bet_to_call > 0 and pot_size > 0:
                    # Calculate pot odds
                    pot_odds = bet_to_call / (pot_size + bet_to_call)
                    # If our win probability is reasonably close to required equity, call instead
                    if win_probability >= (pot_odds * 0.8):  # Call if we have at least 80% of required equity
                        action = 'call'
                        amount = bet_to_call
                        reasoning += f" [POT ODDS SAFEGUARD: {win_probability:.1%} vs {pot_odds:.1%} required - calling]"
                        self.logger.warning(f"POT ODDS SAFEGUARD: Changed fold to call with win_prob={win_probability:.2%}, required={pot_odds:.2%}")
                
                # POSITION SAFEGUARD: Be more aggressive in late position when it's cheap
                elif action == 'fold' and position in ['BTN', 'CO'] and bet_to_call > 0:
                    # If it's a small bet relative to pot in good position, consider calling
                    if bet_to_call <= (pot_size * 0.3) and win_probability >= 0.25:  # 25% win rate with small bet
                        action = 'call'
                        amount = bet_to_call
                        reasoning += f" [POSITION SAFEGUARD: Good position + small bet - calling]"
                        self.logger.warning(f"POSITION SAFEGUARD: Calling in {position} with {win_probability:.2%} win rate")
            
                decision_record = {
                    'action': action,
                    'amount': amount,
                    'reasoning': f"IMPROVED PREFLOP: {reasoning}",
                    'context': {
                        'hand_id': table_data.get('hand_id'),
                        'street': game_stage,
                        'position': position,
                ...
                self.logger.info(f"Improved preflop decision: {action} {amount:.2f} - {reasoning}")
                return decision_record
            
            # POSTFLOP STRATEGY
            elif game_stage in ['flop', 'turn', 'river']:
                action, amount, reasoning = self.improved_postflop_engine.make_postflop_decision(
                    game_state=game_analysis,
                    action_history=self.action_history,
                    street=game_stage
                )
                ...
                decision_record = {
                    'action': action,
                    'amount': amount,
                    'reasoning': f"IMPROVED POSTFLOP: {reasoning}",
                    'context': {
                        'hand_id': table_data.get('hand_id'),
                        'street': game_stage,
                        'position': position,
                ...
                self.logger.info(f"Improved postflop decision: {action} {amount:.2f} - {reasoning}")
                return decision_record
            
            # FALLBACK to original decision engine
            else:
                self.logger.warning(f"Using fallback decision engine for stage={game_stage}, cards={hole_cards}")
                
                # If we can check, always check in fallback mode
                if 'check' in available_actions:
                    action = 'check'
                    amount = 0.0
                    reasoning = "FALLBACK: No hole cards detected, defaulting to check."
                    
                # If we have to call, make better pot odds decisions even without cards
                elif bet_to_call > 0 and pot_size > 0:
                    # Calculate pot odds
                    pot_odds = bet_to_call / (pot_size + bet_to_call)
                    
                    # No hole cards, but we'll use win_probability if available as a fallback
                    if win_probability >= (pot_odds * 0.9):
                        action = 'call'
                        amount = bet_to_call
                        reasoning = f"FALLBACK: No cards but good pot odds ({pot_odds:.1%} required)"
                    elif position in ['BTN', 'CO'] and bet_to_call <= (big_blind * 2):
                        action = 'call'
                        amount = bet_to_call
                        reasoning = f"FALLBACK: No cards but good position and small bet"
                    else:
                        action = 'fold'
                        amount = 0.0
                        reasoning = "FALLBACK: No cards and significant bet to call"
                else:
                    action = 'fold'
                    amount = 0.0
                    reasoning = "FALLBACK: No cards and no check available"
                ...
                self.logger.info(f"Fallback decision: {action} {amount:.2f} - {reasoning}")
                return decision_record
                
        except Exception as e:
            self.logger.error(f"Error in improved strategic decision: {e}", exc_info=True)
            # Fallback to enhanced decision
            return self._make_enhanced_decision(game_analysis)
    
    def _update_action_history(self, action: str, amount: float, player: str, position: str, street: str):
        """Update action history with more detailed information."""
        ...
        action_record = {
            ...
            'street': street.lower(),
            'timestamp': time.time(),
            'hand_id': getattr(self, 'current_hand_id_for_history', None),
            'pot_size_before': getattr(self, 'last_known_pot_size', 0.0)
        }
        
        # Add to action history
        self.action_history.append(action_record)
        
        # Keep only recent actions (last 50 to avoid memory issues)
        if len(self.action_history) > 50:
            self.action_history = self.action_history[-50:]
        
        # Track our own actions for this hand
        if player == self.get_my_player_name():
            self.current_hand_actions.append(action_record)
        
        # Update opponent tracker with this action
        if self.opponent_tracker_enhanced and player != self.get_my_player_name():
            try:
                opponent_profile = self.opponent_tracker_enhanced.get_or_create_opponent(player)
                # Update opponent stats based on action
                was_aggressor = action.upper() in ['BET', 'RAISE']
                if hasattr(opponent_profile, 'update_stats'):
                    opponent_profile.update_stats(action.lower(), street.lower(), was_aggressor)
            except Exception as e:
                self.logger.warning(f"Error updating opponent tracker for {player}: {e}")
        
        self.logger.debug(f"Action history updated: {action_record}")
    
    def get_my_player_name(self) -> str:
        """Get our player name for action tracking."""
        my_player = self.get_my_player()
        if my_player:
            return my_player.get('name', 'Hero')
        return 'Hero'
# Main execution
