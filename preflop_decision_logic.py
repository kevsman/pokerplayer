...
def make_preflop_decision(
    ...
    if preflop_category == "Premium Pair":
        if bet_to_call == 0:
            return action_raise_const, min(raise_amount * 1.1, my_stack)
        elif bet_to_call <= my_stack * 0.45:
            ...
            if ev_raise > ev_call and raise_amount < my_stack * 0.85 :
                return action_raise_const, raise_amount
            elif win_probability > pot_odds_to_call or win_probability > 0.38:
                return action_call_const, bet_to_call
            else:
                return action_fold_const, 0
        else: 
            if win_probability > pot_odds_to_call or win_probability > 0.45:                     
        ...
        win_prob_threshold_open = 0.22 if (is_aj_offsuit or is_kq_offsuit) else 0.25
        win_prob_threshold_call = 0.20 if (is_aj_offsuit or is_kq_offsuit) else 0.23
        ...
        if bet_to_call == 0 and win_probability > win_prob_threshold_open:
            ...
            if win_probability > win_prob_threshold_open + 0.05 and raise_amount < my_stack * 0.5:
                # Consider EV for raise vs call here
                # ev_call = calculate_expected_value_func(...)
                # ev_raise = calculate_expected_value_func(...)
                # if ev_raise > ev_call: return action_raise_const, raise_amount
                return action_raise_const, raise_amount # Simplified: prefer raise if significantly stronger
            return action_call_const, bet_to_call
        elif can_check:
